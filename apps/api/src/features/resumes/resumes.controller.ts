...
export const resumesController = {
   ...
   * @input multipart/form-data with file field `resume` and body fields: jobRole (required), optional jobDescription, aiProvider, keys, userId, jobUrl.
  ...
  async analyze(req: Request, res: Response): Promise<void> {
    ...
    try {
      if (!req.file) {
        throw new ApplicationException('Resume file is required', 400, 'RESUME_FILE_REQUIRED');
      }

      const resumeFile = req.file;
      const { jobRole, jobDescription, geminiKey, groqKey, hfKey, aiProvider, userId, jobUrl } = req.body as {
        jobRole: string;
      ...
      const providerResult = resumeService.resolveProviderAndKey(aiProvider, geminiKey, groqKey, hfKey);

      if (!allowedMimeTypes.includes(resumeFile.mimetype)) {
        throw new ApplicationException('Only PDF and Word documents are supported', 400, 'INVALID_FILE_TYPE');
      }

      tempPath = resumeFile.path || null;
      const fileBuffer = tempPath && fs.existsSync(tempPath) ? fs.readFileSync(tempPath) : null;

      if (!fileBuffer || fileBuffer.length === 0) {
        throw new ApplicationException('File upload failed. Received empty file.', 400, 'EMPTY_FILE');
      }

      let resumeText = '';

      try {
        resumeText = await resumeService.extractWithAIService(fileBuffer, resumeFile.originalname, resumeFile.mimetype);
      } catch {
        // Fallback is handled below.
      }

      if (!resumeText || resumeText.trim().length < 100) {
        resumeText = await resumeService.extractTextFromBuffer(fileBuffer, resumeFile.originalname);
      ...
      const savedFilePath = path.join(resumeService.uploadsDir, `${resume.id}${ext}`);
      fs.writeFileSync(savedFilePath, fileBuffer);

      try {
        const { provider, apiKey } = providerResult;
        const analysis = await resumeService.runAIAnalysis(provider as AIProvider, apiKey, resumeText, jobRole, jobDescription);
        await resumeService.persistAnalysis(resume.id, analysis.result, resumeText, analysis.modelName);
      } catch (err) {
      ...
      res.json(Result.ok('Analysis complete', { id: resume.id, message: 'Analysis complete' }));
    } catch (error) {
  ...
  async getFile(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!resume) {
        res.status(404).json(Result.fail('Resume not found', 'RESUME_NOT_FOUND'));
        return;
      }
      ...
      if (!fs.existsSync(filePath)) {
        res.status(404).json(Result.fail('File not available. It may have been submitted before file storage was enabled.', 'FILE_NOT_FOUND'));
        return;
      }
  ...
  async getById(req: Request, res: Response): Promise<void> {
    try {
      ...
      const ext = path.extname(resume.fileName);
      const filePath = path.join(resumeService.uploadsDir, `${resume.id}${ext}`);
  ...
  /**
   * @description List analysis history, optionally filtered by user ID.
   * @input req.query: { userId?: string }
  ...
  async remove(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.json(Result.ok('Resume deleted successfully', { message: 'Deleted' }));
    } catch (error) {
